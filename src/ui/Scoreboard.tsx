      ...
      <tbody>
        {game.players.map((p, i) => (
          <tr key={i} className={win.includes(i) ? 'winner' : ''}>
            ...
            <td>
              {scores[i].foxes}×{Math.min(scores[i].yellow, scores[i].blue, scores[i].green, scores[i].orange, scores[i].purple)}
            </td>
            <td>
              <strong>{scores[i].total}</strong>
              {final && game.solo && <div className="rating">{soloRating(scores[i].total)}</div>}
            </td>
          </tr>
        ))}
      </tbody>
